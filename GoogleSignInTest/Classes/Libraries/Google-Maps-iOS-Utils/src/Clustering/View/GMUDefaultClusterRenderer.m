...
#import "GoogleSignInTest-Bridging-Header.h"


// Clusters smaller than this threshold will be expanded.
static const NSUInteger kGMUMinClusterSize = 3;
...
static const float kGMUMaxClusterZoom = 10;
...
- (GMSMarker *)markerWithPosition:(CLLocationCoordinate2D)position
  ...
  marker.icon = [UIImage imageNamed:@"terminalPin"];
//    Terminal *terminal = [Terminal new];
  ...
  marker.map = _mapView;
    ...
    if (_mapView.selectedMarker != nil &&
        _mapView.selectedMarker.position.latitude == marker.position.latitude &&
        _mapView.selectedMarker.position.longitude == marker.position.longitude) {
        marker.icon = [UIImage imageNamed:@"terminalPinSelected"];
